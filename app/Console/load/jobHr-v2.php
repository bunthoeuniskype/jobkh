        ...
        //category 
         ...
         foreach ($locationTextArr as $key => $val) {
            if(!City::where('name_compare',$val)->exists()){
             //save en
            $ci = new City;
            $ci->name = $val;//$tr->setSource($transFrom)->setTarget('en')->translate($val);
            $ci->name_compare = $val;
            $ci->country_id = 1;
            $ci->locale = 'zh-CN';
            $ci->save();
         ...
         foreach ($catTextArr as $key => $val) {
           ...
           foreach ($listContent->find('table[class="main-job-list-tab"] tbody tr td div[class="jobtitlelist"] a') as $key => $value) {
                ...
                foreach ($contentDetail->find('div[class="container"] div[class="main-job main-job-line"]') as $val) {
                      ...
                      if($location !== ''){
                        ...
                        if(count($city)>0){
                            $city_id = $city->id;
                             ...
                             //save en
                            $ci = new City;
                            $ci->name = $location1;//$tr->setSource($transFrom)->setTarget('en')->translate($val);
                    ...
                    $cont->level =  $level;//?$tr->setSource($transFrom)->setTarget('en')->translate($level):'';
                    $cont->hiring = $hiring;
                    $cont->salary = $salary;//?$tr->setSource($transFrom)->setTarget('en')->translate($salary):'';
                    $cont->sex = $sex;//?$tr->setSource($transFrom)->setTarget('en')->translate($sex):'';
                    $cont->age = $age;
                    $cont->term = $term;//?$tr->setSource($transFrom)->setTarget('en')->translate($term):'';
                    $cont->function = $function;//?$tr->setSource($transFrom)->setTarget('en')->translate($function):'';
                    $cont->industry = $industry;//?$tr->setSource($transFrom)->setTarget('en')->translate($industry):'';
                    $cont->qualification = $qualification;//?$tr->setSource($transFrom)->setTarget('en')->translate($qualification):'';
                    $cont->language = $language;//?$tr->setSource($transFrom)->setTarget('en')->translate($language):'';
                    $cont->location = $location;//?$tr->setSource($transFrom)->setTarget('en')->translate($location):'';
                    ...
                    $cont->type = $type;//?$tr->setSource($transFrom)->setTarget('en')->translate($type):'';
